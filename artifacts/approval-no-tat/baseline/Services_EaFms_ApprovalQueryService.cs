...
public sealed class ApprovalQueryService(EaFmsDbContext db, IApprovalAuthorizationService authorization, IApprovalDocumentService documents, ICurrentUserService user)
{
    ...
    public async Task<ApprovalListResponseDto> ListAsync(string? search, string? status, string? priority, string? approver, string? requestedBy, string? department, DateTime? createdFrom, DateTime? createdTo, DateTime? requiredFrom, DateTime? requiredTo, string? dueState, int page, int pageSize, CancellationToken ct)
    {
        ...
        if (string.Equals(dueState, "Overdue", StringComparison.OrdinalIgnoreCase)) query = query.Where(x => x.EaTask.CreatedDate.AddMinutes(x.EaTask.AllottedTatMinutes) < now);
        else if (string.Equals(dueState, "Due", StringComparison.OrdinalIgnoreCase)) query = query.Where(x => x.EaTask.CreatedDate.AddMinutes(x.EaTask.AllottedTatMinutes) >= now);

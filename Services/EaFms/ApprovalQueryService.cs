...
public sealed class ApprovalQueryService(EaFmsDbContext db, IApprovalDocumentService documents, ITaskReviewService taskReview)
{
    ...
    public async Task<ApprovalListResponseDto> ListAsync(string? search, string? status, string? priority, string? approver, string? requestedBy, string? department, DateTime? createdFrom, DateTime? createdTo, DateTime? requiredFrom, DateTime? requiredTo, string? dueState, int page, int pageSize, CancellationToken ct)
    {
        ...
        return new() { TotalCount = total, Page = page, PageSize = pageSize, Items = rows.Select(x => ToList(x.Request, x.DocumentCount, now, reviewSummaries.GetValueOrDefault(x.Request.EaTaskId))).ToList() };
    }
    ...
    public async Task<ApprovalDetailDto?> DetailAsync(long id, CancellationToken ct)
    {
        ...
        return ToDetail(request, cycles, await documents.ListAsync(id, ct), await HistoryAsync(request, ct), now, reviewSummary);
    }
